import { IFilters, IButton } from '../interfaces/filterTypes'
import { ITicket } from '../interfaces/ticketsTypes'

// Функция для фильтрации билетов по количеству остановок
function filterByStops(data: ITicket[], filters: IFilters): ITicket[] {
  // Собираем ключи из filters, которые имеют значение true
  const totalStops = Object.keys(filters).filter((key) => filters[key as keyof IFilters])
  ...
  return data.filter((ticket) => {
    return ticket.segments.every((item) => {
      ...
      return stopsCount ? totalStops.includes(stopsCount) : false
    })
  })
}

// Функция для сортировки билетов в зависимости от активной кнопки
function sortTickets(tickets: ITicket[], activeButton: IButton[]): ITicket[] {
...
function filterTickets(data: ITicket[], filters: IFilters, activeButton: IButton[]) {
