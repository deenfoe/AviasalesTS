...
import { ITicketSortState, ITicketSorterItem } from '../../interfaces/sortTypes'

const initialState: ITicketSortState = {
  ticketSorter: [
    {
      id: 1,
      label: 'САМЫЙ ДЕШЕВЫЙ',
...
const ticketSortSlice = createSlice({
  ...
  reducers: {
    setActiveButton: (state, action) => {
      ...
      return {
        ...state,
        ticketSorter: state.ticketSorter.map((label: ITicketSorterItem) => ({
          ...label,
...
export const selectButtons = (state: { ticketSort: ITicketSortState }) => state.ticketSort.ticketSorter
